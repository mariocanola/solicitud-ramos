...
class Solicitud
{
    ...
    /**
     * Solicitudes del periodo actual agrupadas por sede.
     * Usa la misma logica de mapeo fecha->periodo que getResumen para que coincida
     * con los cupos mostrados (corte de 6 AM en weekly, dia 1 en monthly).
     */
    public function getSolicitudesPeriodoActualPorSede($tipo = 'monthly')
    {
        if ($tipo === 'weekly') {
            $periodo = DateHelper::getCurrentPeriodStart('weekly');
            $sql = "SELECT se.nombre as sede_nombre, COUNT(*) as total
                    FROM solicitudes s
                    INNER JOIN sedes se ON se.id = s.id_sede
                    WHERE DATE(
                        CASE WHEN TIME(s.fecha_solicitud) = '00:00:00'
            ...
            $sql = "SELECT se.nombre as sede_nombre, COUNT(*) as total
                    FROM solicitudes s
                    INNER JOIN sedes se ON se.id = s.id_sede
                    WHERE DATE_FORMAT(s.fecha_solicitud, '%Y-%m-01') = ?
                    GROUP BY s.id_sede, se.nombre
                    ORDER BY total DESC";
        }
        $stmt = $this->db->prepare($sql);
